...
class PentagonalBipyramid:
    def __init__(
        ...
        eq = [glm.vec3(math.cos(i * 2 * math.pi / 5) * r, 0.0, math.sin(i * 2 * math.pi / 5) * r) for i in range(5)]
        ...
        for i in range(5):
            vertices += face(eq[i], eq[(i + 1) % 5], v_top)
        for i in range(5):
            vertices += face(eq[(i + 1) % 5], eq[i], v_bot)
